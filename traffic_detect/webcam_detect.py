...
class UDPClient:
    def __init__(self, ip, port):
        ...
        print(f"UDP 發送器初始化，目標：{self.server_ip}:{self.server_port}")
...
if __name__ == "__main__":
    ...
    engine_file_path = "yolov5_traffic.engine" #build/yolov5s.engine
    ...
    # load labels of traffic sign
    categories = ['car', 'carmotorcycle', 'childrencross', 'diverge', 'doublebendleft', 'doublebendright', 'hump', 'leftbend', 'leftmerge', 'lightson', 'max100', 
                  'max110', 'max40', 'max50', 'max60', 'max70', 'max80', 'max90', 'min60', 'nocar', 'noentry', 'nopark', 'nostop', 'noturnleft', 'noturnleftright', 'noturnright', 'nouturn',
                  'pedestriancross', 'rightbend', 'rightmerge', 'signalahead', 'slow', 'speedcamera', 'thisway', 'twostageturn']
    ...
    try:
        ...
        while True:
            ...
            # 顯示辨識結果
            cv2.imshow("YOLOv5 Webcam Detection", batch_image_raw[0])

            # 傳送辨識結果到樹莓派
            result_boxes, result_scores, result_classid = yolov5_wrapper.post_process(
                yolov5_wrapper.host_outputs[0], frame.shape[0], frame.shape[1]
            )

            if len(result_classid) > 0:
                detected_objects = [
                    f"{categories[int(class_id)]}" 
                    for class_id in result_classid
                ]
